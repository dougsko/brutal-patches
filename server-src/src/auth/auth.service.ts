...
export class AuthService {
  ...
  async validateUser(username: string, pass: string): Promise<any> {
    try {
      ...
      if (!isPasswordValid) {
        this.logger.warn(`Invalid password attempt for user: ${username}`, 'AuthService');
        this.logger.logAuth('login_failed', username, false, { reason: 'invalid_password' });
      ...
      this.logger.error(`Error validating user ${username}:`, error.message, 'AuthService', {
        stack: error.stack,
      });
  ...
  async login(user: User) {
    try {
      ...
      this.logger.error(`Login error for user ${user.username}:`, error.message, 'AuthService');
  ...
  async refreshToken(user: User) {
    try {
      this.logger.log(`Token refresh for user: ${user.username}`, 'AuthService');
      ...
      this.logger.error(`Token refresh error for user ${user.username}:`, error.message, 'AuthService');
      this.logger.logAuth('token_refresh_failed', user.username, false, { error: error.message });
